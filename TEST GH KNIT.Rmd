...
```{r import, eval=FALSE, echo=TRUE, message=FALSE, warning=FALSE}
...
```{r bind, eval=FALSE, echo=TRUE, message=FALSE, warning=FALSE}
...
```{r data_check, eval=FALSE, echo=TRUE, message=FALSE, warning=FALSE}
library(dplyr)
library(skimr)
...
All data was imported and binded properly and it is organized in 5 632 443 rows and 19 columns, all rows complete_rate is equal to 1 (which indicates that it is 100% complete). Except end_station_id row (complere_rate = 0,841), the reason of that was mentioned before. 

Data check showed that started_at, ended_at and ride_len columns were imported as characters instead of datetime and duration data types. So it needs to be changed:


## DATA TYPES CHANGE

```{r data_type_changes, eval=FALSE, echo=TRUE, message=FALSE, warning=FALSE}
library(tidyverse)
library(lubridate)
...
As all data were merged into one file, additional month column will be needed for analysis:
...
#ANOTHER THING IS THAT IN EACH MONTH MEMBER USERS PERFORMED MORE RIDES THAN CASUAL USERS
